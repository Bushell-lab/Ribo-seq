...
#This script uses bbmap to align reads. in specificies the input fastq file. out specificies the output SAM file.
#ref specifies the fasta file to use as a reference. bbmap will quickly use this to make an index. As this is mch quicker than other alignment programs, we use the nodisk option so that this isn't written to file
#outm and outu specificy filenames to write all reads that either align or no not align respecitvly in fastq format
...
#We first align to rRNA, then use everything that did not align to rRNA as input to align to tRNAs, then mitochondrial mRNAs and finally protein coding mRNAs
