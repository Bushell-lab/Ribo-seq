...
calculate_single_codon_delta <- function(alist, control = control, treatment = treatment, paired_data = T) {
  do.call("rbind", alist) %>%
    spread(key = condition, value = single_codon_cpm) %>%
...
splice_single_nt <- function(df, region_lengths, codons = 50, UTR_nts = 48, region_cutoffs = c(50,300,50)) {
  ...
  filtered_data[filtered_data$region == "UTR5",] %>%
    ...
    summarise(single_nt_cpm = mean(CPM)) %>%
    ungroup() -> UTR5_start
...
single_codon <- function(df, region_lengths) {
  df %>%
    ...
    summarise(single_codon_cpm = mean(CPM)) %>%
    ungroup() %>%
...
plot_single_transcripts <- function(gene, dir,
  ...
  if (plot_binned == T) {
    ...
    #plot delta
    if (plot_delta == T) {
      binned_delta_data <- calculate_binned_delta(binned_list, value = "binned_cpm", control = control, treatment = treatment, paired_data = paired_data)
  ...
  if (plot_single_nt == T) {
    ...
    if (plot_delta == T) {
      #calculate and plot delta
      single_nt_delta_data <- calculate_single_nt_delta(single_nt_list, value = "single_nt_cpm", control = control, treatment = treatment, paired_data = paired_data)
  ...
  if (plot_codons == T) {
    ...
    summarised_codon_list <- lapply(single_codon_list, summarise_data, value = "single_codon_cpm", grouping = "codon")
...
plot_single_nt_heatmaps <- function(gene_names, remove_IDs, col_lims) {
  ...
  single_transcript_data %>%
    ...
    select(all_of(gene_sym, condition, replicate, plot_position, cpm)) %>%
