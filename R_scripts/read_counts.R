...
for (i in 1:nrow(read_counts)) {
  ...
  split_fylename <- str_split(fylename, "_")
  
  fylename_list[[fylename]] <- data.frame(fylename = fylename,
                            sample = split_fylename[[1]][1],
...
#plot total counts----
RPF_counts %>%
  ggplot(aes(x = sample, y = all, fill = replicate))+
...
RPF_counts %>%
  ...
  ggplot(aes(x = sample, y = unique_perc, fill = replicate))+
  geom_col(position = position_dodge())+
  ylab("trimmed reads %")+
  ylim(c(0,100))+
...
RPF_counts %>%
  ...
  ggplot(aes(x = sample, y = rRNA_perc, fill = replicate))+
  geom_col(position = position_dodge())+
  ylab("rRNA reads %")+
  ylim(c(0,100))+
  ggtitle("RPFs rRNA")+
...
RPF_counts %>%
  mutate(non_rRNA_counts = cdhitdup - rRNA,
         ...
         sample_name = paste(sample, replicate, sep = " ")) %>%
  select(sample_name, pc_perc, tRNA_perc, mito_perc, unaligned_perc) %>%
  ...
  ggplot(aes(x = sample_name, y = percentage, fill = alignment))+
  geom_col()+
  xlab("sample")+
  ylab("% aligments")+
