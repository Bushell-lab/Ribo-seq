...
do
counting_script.py -bam $BAM_dir/${filename}_pc_best_sorted.bam -fasta $pc_fasta -len $lengths -offset $offsets -out_file ${filename}_pc_best_final_codon.counts -out_dir $counts_dir &
done
wait
...
for filename in $RPF_filenames
do
count_codon_occupancy.py ${filename}_pc_best_final_codon.counts $pc_fasta $region_lengths -in_dir $counts_dir -out_dir $codon_counts_dir &
done
wait
